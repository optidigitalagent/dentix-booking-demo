...
import { selectSurgeryPrices, surgeryPages, type SurgeryRoute } from "@/data/surgery-pages";
import { siteHref } from "@/lib/site-href";

export function SurgeryPage({ route }: { route: SurgeryRoute }) {
  const page = surgeryPages[route];
  const { priceBlocks, priceSource } = useManagedContent();
  const prices = selectSurgeryPrices(route, priceBlocks);
  return (
    <SiteLayout>
      <section className="price-hero therapy-hero" style={{ "--price-hero-image": `url(${clinicImage})` } as CSSProperties}>
        <div className="wrap price-hero-grid"><div className="price-hero-copy">
          <nav className="breadcrumbs" aria-label="Навігація">
            ...
            {route !== "surgery" && <><a href={siteHref("/" + surgeryPages.surgery.path)}>{surgeryPages.surgery.label}</a><span aria-hidden="true"> / </span></>}
          ...
          <div className="hero-actions">
            ...
            <BookingButton className="btn btn-ghost" requestedInterest="Хірургія">Узгодити прийом</BookingButton>
          </div>
      ...
      <section className="section" id="service-scope">
        <div className="wrap therapy-overview">
          <div>
            ...
            <h2 className="sec-title">{route === "surgery" ? "Послуги з видалення зубів" : page.label}</h2>
            <ul className="therapy-scope">{page.scope.map((label) => <li key={label}>{label}</li>)}</ul>
            {route === "surgery" && <nav className="entity-links" aria-label="Послуги напрямку">{page.related.map((key) => <a key={key} href={siteHref("/" + surgeryPages[key].path)}>{surgeryPages[key].label} →</a>)}</nav>}
          </div>
          <div>
      ...
      {route === "extraction" && <section className="section" id="complex-extraction">
        <div className="wrap">
          <h2 className="sec-title">Складне видалення ретинованого зуба</h2>
          <p className="sec-lede">Це окрема позиція поточного прайсу DENTIX. Обсяг послуги та остаточну вартість підтверджує клініка після огляду й діагностики.</p>
          <a className="price-booking-link" href="#service-prices">Переглянути вартість послуг →</a>
        </div>
      </section>}
      ...
      <TeamSection surgeryRoute={route} />
      ...
      <section className="section" id="related-services">
        <div className="wrap">
          ...
          <nav className="therapy-related" aria-label="Послуги хірургічного напрямку">
            {page.related.map((key) => <a className="btn btn-ghost" key={key} href={siteHref("/" + surgeryPages[key].path)}>{surgeryPages[key].label} →</a>)}
          </nav>
