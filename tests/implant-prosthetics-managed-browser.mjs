...
try {
  ...
  for (const target of ["preview", "production"]) {
    ...
    const temp = await fs.mkdtemp(path.resolve("node_modules/.dentix-implant-prosthetics-qa-"));
    try {
      ...
      for (const [route, service] of Object.entries(implantProstheticsPages)) for (const mode of ["valid", "valid-comma", "empty", "invalid", "unmatched", "near-role", "cross-role"]) {
        const context = await browser.newContext({ viewport: { width: 390, height: 900 }, reducedMotion: "reduce" });
        try {
          const fixtureDoctor = { id: "albert-podolyansky", full_name: "QA managed clinician", role: mode === "unmatched" ? "Лікар-терапевт" : mode === "near-role" ? (route === "implantation" ? "імплантологія" : "Лікар-ортодонт") : mode === "cross-role" ? (route === "implantation" ? "Лікар-ортопед" : "Лікар-імплантолог") : (mode === "valid-comma" ? "стоматолог, " : "Лікар-") + (route === "implantation" ? "імплантолог" : "ортопед") };
          const fixturePrices = selectImplantProstheticsPrices(route, priceBlocks).map((row, index) => ({ id: "qa-" + index, category_id: "hirurgiya", category: "Хірургічна стоматологія", service_name: row.name, price: "QA price " + index, price_note: "QA current note", sort_order: index }));
          ...
          const remote = ["valid", "valid-comma", "unmatched", "near-role", "cross-role"].includes(mode);
          ...
          if (["valid", "valid-comma"].includes(mode)) {
          ...
          } else if (["near-role", "cross-role"].includes(mode)) {
            assert.deepEqual(await page.locator("#service-prices .price-cost").allTextContents(), fixturePrices.map((row) => row.price));
            ...
            assert.ok(await page.getByText("Уточніть актуальну вартість цих послуг у клініці телефоном.").isVisible());
            assert.equal(await page.locator("#team .doc").count(), 0);
            assert.ok(await page.getByText("Уточніть лікаря цього напрямку у клініці телефоном.").isVisible());
          } else {
  ...
  await fs.writeFile(path.join(output, "implant_prosthetics_managed_content.json"), JSON.stringify({ results, violations, synthetic: true, pass: results.length === 28 && violations.length === 0 }, null, 2));
}
