...
import { therapyPages, selectTherapyPrices } from "../src/data/therapy-pages.ts";
...
try {
  ...
  for (const target of ["preview", "production"]) {
    ...
    const temp = await fs.mkdtemp(path.resolve("node_modules/.dentix-therapy-qa-"));
    try {
      ...
      try {
        ...
        for (const [route, page] of Object.entries(therapyPages)) {
      ...
      for (const [route, service] of Object.entries(therapyPages)) for (const mode of ["valid", "empty", "invalid", "unmatched"]) {
        const context = await browser.newContext({ viewport: { width: 390, height: 900 }, reducedMotion: "reduce" });
        try {
          const fixtureDoctor = { id: "albert-podolyansky", full_name: "QA managed clinician", role: mode === "unmatched" ? "Лікар-ортодонт" : "Лікар-терапевт, ендодонтист, мікроскопіст" };
          const fixturePrices = selectTherapyPrices(route, priceBlocks).map((row, index) => ({ id: "qa-" + index, category_id: "terapiya", category: "Терапевтична стоматологія", service_name: row.name, price: "QA price " + index, price_note: "QA current note", sort_order: index }));
          ...
          const remote = ["valid", "unmatched"].includes(mode);
          ...
          if (mode === "valid") {
            ...
            assert.deepEqual(await page.locator("#service-prices .price-cost").allTextContents(), selectTherapyPrices(route, priceBlocks).map((row) => row.cost));
            assert.equal(await page.locator("#team .doc").count(), route === "microscope" ? 1 : 2);
          }
  ...
  await fs.writeFile(path.join(output, "therapy_managed_content.json"), JSON.stringify({ results, violations, synthetic: true, pass: results.length === 24 && violations.length === 0 }, null, 2));
}
