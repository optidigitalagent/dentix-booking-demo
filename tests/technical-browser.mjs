...
try {
  for (const target of ["preview", "production"]) {
    ...
    try {
      ...
      for (const width of [360, 390, 768, 1024, 1440]) for (const file of ["", "price.html"]) {
        ...
        const primary = await page.locator(file ? ".price-list" : "#services").innerText();
        assert.ok(primary.length > 300);
      ...
      for (const width of [360, 390, 768, 1024, 1440]) {
        for (const file of ["", "price.html"]) {
          ...
          const screenshot = `${target}-${file ? "price" : "home"}-${width}.png`;
