...
const semantics = [];
const servicePaths = ["terapevtychna-stomatolohiia/", "likuvannia-kariiesu/", "lechenie-pod-mikroskopom/", "khirurhichna-stomatolohiia/", "vydalennia-zuba/", "vydalennia-zuba-mudrosti/", "implantatsiya/", "protezirovanie/"];
...
try {
  for (const target of ["preview", "production"]) {
    ...
    try {
      ...
      for (const width of [360, 390, 768, 1024, 1440]) for (const file of patientPaths) {
      ...
      for (const width of [360, 390, 768, 1024, 1440]) {
        for (const file of patientPaths) {
          ...
          assert.equal(await page.locator('a[aria-current="page"]').count(), servicePaths.includes(file) ? 0 : 1);
          ...
          if (width < 1024) {
            ...
            assert.equal(await page.locator('#nav-mobile a[aria-current="page"]').count(), servicePaths.includes(file) ? 0 : 1);
            await page.keyboard.press("Escape");
          ...
          if (file) await page.locator(".breadcrumbs a").first().click();
