...
export async function checkLeadSourceEntryPoints({ browser, output }) {
  ...
  try {
    ...
    for (const target of ["preview", "production"]) {
      ...
      for (const file of ["likari/", "kontakty/", "price.html", "terapevtychna-stomatolohiia/", "likuvannia-kariiesu/", "lechenie-pod-mikroskopom/", "khirurhichna-stomatolohiia/", "vydalennia-zuba/", "vydalennia-zuba-mudrosti/"]) assets.set(base + file, assets.get(base + "index.html"));
      ...
      for (const file of ["", "likari/", "kontakty/", "price.html", "terapevtychna-stomatolohiia/", "likuvannia-kariiesu/", "lechenie-pod-mikroskopom/", "khirurhichna-stomatolohiia/", "vydalennia-zuba/", "vydalennia-zuba-mudrosti/"]) for (const catalogReady of [false, true]) {
    ...
    await fs.writeFile(path.join(output, "lead_source_entry_points.json"), JSON.stringify({ observed_at: new Date().toISOString(), fixture_scope: "Real source; in-memory builds; all network intercepted; no request or personal data submitted", results, requests, pass: results.length === 40 }, null, 2) + "\n");
  }
}
