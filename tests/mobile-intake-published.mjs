...
const origin='https://optidigitalagent.github.io/dentix-booking-demo';
const dir='docs/qa/mobile-intake-2026-09-07';await fs.mkdir(dir,{recursive:true});const results=[];
for(const [engineName,engine] of Object.entries({chromium,webkit})){
 const browser=await engine.launch();try{for(const [width,height] of [[360,844],[375,812],[390,844],[393,852],[412,915],[430,932],[844,390],[768,900],[1024,900],[1440,900]]){
  ...
  if(width===390){await page.locator('.lead-form').first().scrollIntoViewIfNeeded();await page.screenshot({path:`${dir}/published-${engineName}-390.png`});await page.locator('footer.footer').scrollIntoViewIfNeeded();await page.waitForTimeout(150);assert.equal(await page.locator('.mobile-call-bar').count(),0);}
